...
def load_models():
    ...
    with st.spinner("Downloading Stage 1 model..."):
        download_from_gdrive(STAGE1_GDRIVE_ID, s1_path)
    with st.spinner("Downloading Stage 2 model..."):
...
def load_dicom_files(uploaded_files, img_size=224):
    import pydicom
    ...
    for uf in sorted(uploaded_files, key=lambda x: x.name):
        ...
        try:
            ...
            img_model = cv2.resize(((img_win + 1000) / 1400 * 255), (img_size, img_size)).astype(np.float32)
            slices_model.append(np.stack([img_model]*3, axis=-1))
            fnames.append(uf.name)
        finally:
            os.unlink(tmp_path)
...
def make_bar_chart(det_scores, threshold, nodule_groups):
    ...
    ax.bar(range(len(det_scores)), det_scores, color=colors, edgecolor='none', alpha=0.9, width=0.8)
    ax.axhline(threshold, color='#f8fafc', linestyle='--', linewidth=1.2,
...
uploaded = st.file_uploader(
    "Upload all DICOM (.dcm) files from a single CT scan",
    type=["dcm"],
    accept_multiple_files=True,
    help="Select all .dcm files from one patient scan folder"
)

threshold = st.slider("Detection threshold", 0.3, 0.9, 0.5, 0.05,
                      help="Slices scoring above this are forwarded to Stage 2")

run_btn = st.button("🔬 Run Analysis", disabled=len(uploaded) == 0)

if uploaded and not run_btn:
    st.markdown(f"""
    <div class="warning-box">
        {len(uploaded)} DICOM files loaded — click <strong>Run Analysis</strong> to begin.
    </div>""", unsafe_allow_html=True)
...
if run_btn and uploaded:
    with st.spinner("Loading models..."):
        try:
            ...
            st.error(f"Could not load models: {e}\n\nMake sure `stage1_FULL.keras` and `stage2_FULL.keras` are in the same folder as `app.py`.")
            st.stop()

    with st.spinner(f"Processing {len(uploaded)} DICOM slices..."):
        fnames, vol_display, vol_model = load_dicom_files(uploaded)
    ...
    if region_results:
        ...
        with right:
            # Show up to 6 representative slices
            show = region_results[:6]
            cols = st.columns(min(3, len(show)))
            for idx, (col, res) in enumerate(zip(cols * 2, show)):
                with col:
                    pil_img = slice_to_pil(vol_display[res['best_slice']])
    ...
    <div class="warning-box" style="background:#0f1a0f; border-color:#16653144; color:#86efac">
        ...
        clinical decision is made. Final diagnosis remains under physician supervision.
    </div>
    """, unsafe_allow_html=True)
    ...
    <div class="upload-zone">
        ...
        <div style="color:#475569; font-size:0.9rem">
            Upload all <strong style="color:#64748b">.dcm files</strong> from a single CT scan to begin analysis
        </div>
    </div>
    """, unsafe_allow_html=True)
